...
함수().then((n) => console.log(`함수() 프로미스 객체 반환: ${n}`)); // 안녕
...
함수2().then((n) =>
  console.log(`함수2() async함수는 언제나 프로미스 객체 return: ${n}`)
); //잘가
...
함수3().then((n) =>
  console.log(`함수3() async함수 안에서 return new Promise: ${n}`)
); //안녕하세요
...
더하기().then(function (결과) {
  console.log(`async 함수의 return 값은 then으로 받을 수 있다: ${결과}`); //2
});
...
// new Promise의 반환값 = resolve에 전달되는 인자
async function 더하기2() {
  var 어려운연산 = new Promise((성공, 실패) => {
    setTimeout(() => {
      var 결과 = 10 + 10;
...
더하기2().then((a) => {
  console.log(`async 더하기2 함수의 return 값을 then으로 받는다: ${a}`);
});
